...
This program is for the Assignment2 task4.
To read both of the txt file which was generated before and Use their data to make tables
then select the table find biggest probability to forecast the next word of users' input.
"""
...
class BiGramModel:
    ...
    def __init__(self, f_name):
        ...
        # Call the g_statement function with user input as statement
        self.g_statement(input('\nPlease type in statement: '))
    ...
    def g_statement(self, statement):
        # split the sentence with space and the last item is what we want
        last_word = statement.lower().split(' ')[-1]
        # I was inspired by pandas(2020) Indexing and selecting data
        # Reference: https://pandas.pydata.org/pandas-docs/stable/user_guide/indexing.html
        # search the last word of the sentence in our table, then find the biggest probability
        # which is means the most likely occur word in Bi_gram.
        grams = self.create_table.loc[last_word].idxmax()
        print(grams)


# define main function
# forgive me to write it in stupid way, the pandas read_csv function do not work when I test it on Move.
def main():
